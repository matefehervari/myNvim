...
local function on_attach(bufnr)
  ...
  local function file_details()
    local tree_node =  tree.get_node_under_cursor()

    if not tree_node then
        return
    end

    local abs_path = tree_node.absolute_path
    local details = vim.fn.system({"file", "-b", abs_path})
    vim.notify(details)
  end
  ...
  nnoremap('f',     file_details,                   opts('Filter'))
